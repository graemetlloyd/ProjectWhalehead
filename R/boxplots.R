#' Create box plots of our data
#' 
#' This function will take a file of stratigraphic congrunce metrics and make a box plot. Returns nothing.
#' @import ggplot2
...
makeBoxplot <- function(x) {
  ...
  q <- p + geom_boxplot(scale = "count") + geom_jitter(height = 0, width = 0.2, alpha = .4)
  just_names <- strsplit(x, split = '/')[[1]][-1][3]
  n <- just_names[[1]][length(just_names[[1]])] 
  print(n)
  new_filename <- paste0("boxPlots/",n, '.pdf')
