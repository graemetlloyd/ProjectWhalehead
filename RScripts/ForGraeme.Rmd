...
custom_plots <- function (CombinedTrees, StratFile) {
  ...
  # Data to write out for Voronoi plotting:
  dataforvoronoi <- points[, c("x", "y", "bayes", "MIG")]
  # Write out to Voronoi folder:
  write.table(dataforvoronoi, paste(gsub("CombinedTrees", "VoronoiData", file_path_sans_ext(CombinedTrees)), ".txt", sep = ""), row.names = FALSE)
 pp <- ggplot(points, aes(x = x, y = y, color = points$MIG))
final<- pp + geom_point(aes(size=bayes)) + scale_size(range = c(5,1), guide = FALSE)  +   scale_colour_gradient(low = "blue", high = "yellow", guide = guide_colorbar(title = "MIG"))
