...
points = custom_treespace(rwty_obj, 1000)
pp <- ggplot(points %>%
             arrange(bayes),
aes(x = x, y = y, color = points$MIG))
...
files <- list.files(path="Data/StratCongruenceResults/InputTreeResults", pattern="*.log", full.names=TRUE, recursive=FALSE)
#And the list of treefiles
tre_files <-  list.files(path="Data/BayesianTopologies", pattern="*.trees", full.names=TRUE, recursive=FALSE)
...
#Now make the plots
mapply(congruify, ages_files, parsimony_files, tre_files)
mapply(custom_plots, files, mat_files, tre_files, parsimony_files)
```
...
```{r}
congruify <- function (AgeFilePath, ParsimonyTreePath, BayesianTreesPath) {
ParsimonyTrees <- ape::read.tree(ParsimonyTrees)

RawAges <- read.table(ages, header = TRUE, stringsAsFactors = FALSE)
...
if(length(setdiff(sort(rownames(TipAges)), sort(ParsimonyTrees[[1]]$tip.label))) > 0) {
  NameMatches <- do.call(rbind, lapply(apply(cbind(sort(rownames(TipAges)), sort(ParsimonyTrees[[1]]$tip.label)), 1, as.list), function(x) unlist(x)))
    ...
    for(j in 1:nrow(NameMatches)) {
      ...
      class(ParsimonyTrees) <- "multiPhylo"
     RevBayesOutput <- read.table(BayesianTree, header = TRUE, stringsAsFactors = FALSE)
BayesianTrees <- ape::read.tree(text = RevBayesOutput[, "tree"])
    OutgroupTaxon <- rownames(Claddis::ReadMorphNexus(mat)$Matrix_1$Matrix)[1]
    ...
    new_filename <- paste0(BayesianTree, ".nex")
