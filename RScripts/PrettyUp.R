...
fi <- read.table("Data/StratCongruenceResults/InputTreeResults/Yates_2003a.txt", header = TRUE, sep = " ")
...
b <- p + geom_boxplot() + theme_bw() +scale_x_discrete(breaks = c(0,1), labels = c("Parsimony", "Bayes")) + xlab(NULL)
 ...
 grid.arrange(arrangeGrob(b,q, ncol=2, nrow=1), arrangeGrob(yp, ncol=1, nrow=1), heights = c(4,6))
